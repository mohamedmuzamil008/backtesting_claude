...
def calculate_signals(openconviction_5, first1_open, first1_close, sd_atr, volatility, multiplier, low, curbot, time, curtop, atr, high, close, open, entry_param, sl_param, target_param, trail_activation_param, trail_sl_param, uptrend, downtrend, vah, poc, val, openlocation):
    ...
    for i in range(1, n):
        ...
        buy[i] = 1 if ((openlocation[i] == 4 or openlocation[i] == 5) and 
                       low[i] <= curtop[i] - entry_param[i] and
                       downtrend[i] == True and 
...
def generate_strategy10_signals(df, daily_df, results_dir, file_name):

    # Exclude if its not uptrend
    df = df[df['downtrend'] == True].reset_index(drop=True)

    df['time_seconds'] = df['time'].apply(lambda x: x.hour * 3600 + x.minute * 60 + x.second)

    df['entry_param'] = np.where(df['downtrend'] == True, (1 * df['ATR']) + (0.0 * df['range_sd']), df['ATR'] + ((1.8 + df['SD/ATR'])  * df['range_sd']))
    df['sl_param'] = np.where(df['downtrend'] == True, df['entry_param'] + (df['range_sd']), df['entry_param'] + ((0.4 + df['SD/ATR'])  * df['range_sd']))
    df['target_param'] = np.where(df['downtrend'] == True, df['entry_param'] - (0.5 * df['ATR']), df['entry_param'] - ((0.7 + df['SD/ATR'])  * df['range_sd']))
    df['trail_activation_param'] = np.where(df['downtrend'] == True, df['entry_param'] - (0.5 * df['ATR']), df['entry_param'] - ((0.5 + df['SD/ATR']) * df['range_sd']))
    df['trail_sl_param'] = np.where(df['downtrend'] == True, df['entry_param'] - (0.2 * df['ATR']), df['entry_param'] - ((0.3 + df['SD/ATR']) * df['range_sd']))
