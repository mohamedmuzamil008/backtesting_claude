...
def calculate_signals(openconviction_5, first1_open, first1_close, sd_atr, volatility, multiplier, low, curbot, time, curtop, atr, high, close, open, entry_param, sl_param, target_param, trail_activation_param, trail_sl_param, uptrend, vah, poc, val, openlocation):
    ...
    for i in range(1, n):
        ...
        buy[i] = 1 if (openlocation[i] == 4 and 
                       curtop[i] < val[i] and    
                       first1_close[i] < first1_open[i] and
                       close[i] >= first1_open[i] and
                       first1_open[i] < 0.997 * val[i]  and                 
                       uptrend[i] == True and                  
                       multiplier[i] >= 0.0022 and multiplier[i] <= 0.0070 and
                       volatility[i] >= 0.012 and 
                       time[i] > 33300 and time[i] <= 35040 and #33540  
...
def generate_strategy7_signals(df, daily_df, results_dir, file_name):
    ...
    df['trail_activation_param'] = np.where(df['uptrend'] == True, df['VAH_prev'], df['entry_param'] - ((0.5 + df['SD/ATR']) * df['range_sd']))
    df['trail_sl_param'] = np.where(df['uptrend'] == True, df['VAH_prev'] - 0.004 * df['VAH_prev'], df['entry_param'] - ((0.3 + df['SD/ATR']) * df['range_sd']))
