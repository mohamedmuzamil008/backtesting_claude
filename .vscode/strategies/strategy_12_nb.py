...
def generate_strategy12_signals(df, daily_df, results_dir, file_name):
    ...
    df['sl_param'] = np.where(df['uptrend'] == True, np.where(df['ibrange/atr'] <= 0.5, df['IB_low'], df['IB_low'] - 0.25 * df['IB_range']), df['entry_param'] + ((0.4 + df['SD/ATR'])  * df['range_sd']))
