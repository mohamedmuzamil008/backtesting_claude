...
def calculate_signals(openconviction_5, sd_atr, volatility, multiplier, low, curbot, time, curtop, atr, high, close, open, entry_param, sl_param, target_param, trail_activation_param, trail_sl_param, uptrend, vah, poc, val, openlocation):
    ...
    for i in range(1, n):
        ...
        buy[i] = 1 if (openlocation[i] == 4 and 
                       curtop[i] < val[i] and                        
                       low[i] <= entry_param[i] and       
                       low[i] == curbot[i] and 
                       uptrend[i] == True and                  
                       #multiplier[i] >= 0.0022 and multiplier[i] <= 0.0070 and
                       #volatility[i] >= 0.012 and 
                       time[i] > 33300 and time[i] <= 54000 and #33540  
...
def generate_strategy6_signals(df, daily_df, results_dir, file_name):
    ...
    #df['entry_param'] = np.where(df['uptrend'] == True, df['curtop'] - ((0.7 * df['ATR']) + ((0.7 * df['volatility'] * 100)  * df['range_sd'])), df['ATR'] + ((1.8 + df['SD/ATR'])  * df['range_sd']))
    df['entry_param'] = np.where(df['uptrend'] == True, df['curtop'] - (1 * df['ATR']), df['ATR'] + ((1.8 + df['SD/ATR'])  * df['range_sd']))
    ...
    df['sl_param'] = np.where(df['uptrend'] == True, df['entry_param'] - (0.2 * df['ATR']), df['entry_param'] + ((0.4 + df['SD/ATR'])  * df['range_sd']))
    df['target_param'] = np.where(df['uptrend'] == True, df['VAL_prev'], df['entry_param'] - ((0.7 + df['SD/ATR'])  * df['range_sd']))
    df['trail_activation_param'] = np.where(df['uptrend'] == True, df['curtop'], df['entry_param'] - ((0.5 + df['SD/ATR']) * df['range_sd']))
    df['trail_sl_param'] = np.where(df['uptrend'] == True, df['curtop'] - 0.004 * df['curtop'], df['entry_param'] - ((0.3 + df['SD/ATR']) * df['range_sd']))
    ...
    trades_taken_today, buy, trade_active, buy_price, sl_price, tp_price, trail_activated, trail_activation_price, trail_sl_price, \
        ...
        df['openconviction_5'].values, df['SD/ATR'].values, df['volatility'].values, df['multiplier'].values, df['Low'].values, df['curbot'].values,
