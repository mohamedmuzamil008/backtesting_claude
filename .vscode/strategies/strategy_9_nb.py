...
def calculate_signals(openconviction_5, first1_open, first1_close, first1_high, first5_high, sd_atr, volatility, multiplier, low, curbot, time, curtop, atr, high, close, open, entry_param, sl_param, target_param, trail_activation_param, trail_sl_param, uptrend, vah, poc, val, prev_high, prev_low, openlocation):
    ...
    for i in range(1, n):
        ...
        buy[i] = 1 if ((openlocation[i] == 1 or openlocation[i] == 4) and 
                       uptrend[i] == True and 
                       curtop[i] <= 0.996 * vah[i]  and   
                       #curtop[i] < close[i] + (0.7 * atr[i]) and  
                       curbot[i] > prev_low[i] and
                       close[i] > first1_high[i] and
                       multiplier[i] >= 0.0015 and multiplier[i] <= 0.007 and
...
def generate_strategy9_signals(df, daily_df, results_dir, file_name):
    ...
    #df['sl_param'] = np.where(df['uptrend'] == True, np.minimum(0.998 * df['prevday_low'], 0.996 * df['entry_param']), df['entry_param'] + ((0.4 + df['SD/ATR'])  * df['range_sd']))
    df['sl_param'] = np.where(df['uptrend'] == True, 0.998 * df['first1_low'], df['entry_param'] + ((0.4 + df['SD/ATR'])  * df['range_sd']))
    #df['target_param'] = np.where(df['uptrend'] == True, df['prevday_high'] * 1.01, df['entry_param'] - ((0.7 + df['SD/ATR'])  * df['range_sd']))
    df['target_param'] = np.where(df['uptrend'] == True, df['Close'] + (0.6 * df['ATR']), df['entry_param'] - ((0.7 + df['SD/ATR'])  * df['range_sd']))
    #df['trail_activation_param'] = np.where(df['uptrend'] == True, df['prevday_high'] * 1.005, df['entry_param'] - ((0.5 + df['SD/ATR']) * df['range_sd']))
    df['trail_activation_param'] = np.where(df['uptrend'] == True, df['Close'] + (0.6 * df['ATR']), df['entry_param'] - ((0.5 + df['SD/ATR']) * df['range_sd']))
    df['trail_sl_param'] = np.where(df['uptrend'] == True, df['trail_activation_param'] - (0.004 * df['trail_activation_param']), df['entry_param'] - ((0.3 + df['SD/ATR']) * df['range_sd']))
    ...
    trades_taken_today, buy, trade_active, buy_price, sl_price, tp_price, trail_activated, trail_activation_price, trail_sl_price, \
        ...
        df['openconviction_5'].values, df['first1_open'].values, df['first1_close'].values, df['first1_high'].values, df['first5_high'].values,  df['SD/ATR'].values, df['volatility'].values, df['multiplier'].values, df['Low'].values, df['curbot'].values,
        ...
        df['trail_sl_param'].values, df['uptrend'].values, df['VAH_prev'].values, df['POC_prev'].values, df['VAL_prev'].values, df['prevday_high'].values, df['prevday_low'].values, df['Open_Location'].values,
    )
